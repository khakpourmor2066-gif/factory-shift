...
کاربر دارای نقش `HR`، `SUPERVISOR` یا `ADMIN` می‌تواند برنامه روزانه یا ماهانه را
از فایل CSV یا Excel وارد کند. هر ردیف باید به یک کد کارمندی فعال متصل شود.
در رابط مدیریتی فعلی، این فرایند برای `HR` و `ADMIN` در دسترس است.
